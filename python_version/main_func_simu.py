import python_version.tag_gestion_idea as tgi
"""
ONLY FOR DEMO.
"""
N_BYTES = 8
...
def set_tag(tag:str, size:int)->int:
    """aujouté un tags au gestionnaire de tags contenue dans GData2
    ...
    if size == 16:
        ...
        for i in range(N_BYTES):
            if tgi.have_space_for_value(GData2.GT[i][0], 8):
                n_dispo += 1
        if n_dispo < 2:
            return -1 # pas asser d'espace pour mettre le tags
        ...
        for i in range(N_BYTES):
            if tgi.have_space_for_value(GData2.GT[i][0], 8):
                if mode_ == "a":
                    ...
                    GData2.GT[i][0] = f";16a_{tag}"
                elif mode_ == "b":
                    # partie b de la valeur à 16 bits
                    GData2.GT[i][0] = f";16b_{tag}"
                    return 1    # l'ajout a été réussi
    elif size == 8:
        for i in range(N_BYTES):
            if tgi.have_space_for_value(GData2.GT[i][0], 8):
                GData2.GT[i][0] = f";8_{tag}"
                return 1
        return -1 # pas asser de place 
    elif (size == 4) or (size == 3) or (size == 1):
        for i in range(N_BYTES):
            if tgi.have_space_for_value(GData2.GT[i][0], size):
                tgi.add_tag(GData2.GT[i], tag, size)
                return 1
...
def get_n_bytes(tag) -> int:
    ...
    n_byte = -1
    for i in range(N_BYTES):
        if tgi.containe_tag(GData2.GT[i][0], tag):
            n_byte = i
            break
    if n_byte == -1:
        # tag non trouvé
...
def set_value(tag:str, value:int):
    ...
    size = tgi.get_size_tag(GData2.GT[n_byte][0],tag) 
    p0 = tgi.get_position_first_bits(GData2.GT[n_byte][0], tag)
    ...
    bytes_actif = GData2.data[n_byte]
    ...
    if size == 8:
        GData2.data[n_byte] = value
        return 1
    else:
        GData2.data[n_byte] = byte_new
    ...
    #print(f"data avant {beau_print_mask(value_)}\t{(GData2.data[n_byte])}")
    #GData2.data[n_byte] &= value
    #print(f"data sans {beau_print_mask(GData2.data[n_byte])}")
    #print("new data: ", beau_print_mask(value))
    ## set the new data
    #GData2.data[n_byte] = simul_OR(GData2.data[n_byte], val_modif)
    #print(f"data après {beau_print_mask(GData2.data[n_byte])}")
...
def set_value16(tag:str, value:int):
    ...
    # set the value
    GData2.data[index_a] = data_a
    GData2.data[index_b] = data_b
...
def get_index_ab_tag_16(tag:str)->tuple[int, int]:
    ...
    index_b = -1
    for e in range(l0):
        if tgi.containe_tag(GData2.GT[e][0], tag):
            if tgi.get_size_tag(GData2.GT[e][0], tag) != 16:
                # cas de valeur avec un nombre de bit inférieur à 16 demandé à cette fonciton.
                return (e, -1)
            else:
                if f";16a_{tag}" == GData2.GT[e][0]:
                    if index_a != -1:
                        raise ValueError("plus d'un index est présent pour la partie a du tag")
                    index_a = e
                elif f";16b_{tag}" == GData2.GT[e][0]:
                    if index_b != -1:
                        raise ValueError("more than one index is present for the coresponding tag")
                    index_b = e
                else:
                    # juste to be safe
...
def get_value16(tag:str):
    ...
    # get the 2 bytes
    ba = GData2.data[index_a]
    bb = GData2.data[index_b]
...
def get_value(tag:str):
    ...
    gt = GData2.GT[n_byte][0]
    
    # get the bytes where the data is stored
    data = GData2.data[n_byte]
...
def print_all_bata_bin():
    r = "tag:\t"
    for e in GData2.GT:
        r += f"{e}\t"
    print(r)
    r = "DATA: \t"
    for e in GData2.data:
...
def send(l_bytes:list):
    ...
    """
    while len(l_bytes) < len(GData2.data):
        l_bytes.append(0)
    for e in enumerate(GData2.data):
        l_bytes[e[0]] = e[1]
